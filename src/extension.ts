...
class Grammar {
    ...
    constructor(lang: string) {
        ...
        for (let i=0; i< grammarJson.complexTerms.length; i++)
    ...
    async init() {
        // Load wasm parser
        await parserPromise;
		const langObj = await Parser.Language.load(path.join(__dirname, "../parsers", this.lang + ".wasm"));
    ...
    parse(tree: Parser.Tree) {
        ...
        let node : Parser.SyntaxNode | null = tree.rootNode.firstChild;
...
function buildLegend() {
    ...
    termMap.forEach(t => {
        if (!tokenTypes.includes(t.type))
            tokenTypes.push(t.type);
        t.modifiers?.forEach(m => {
            if (!tokenModifiers.includes(m))
                tokenModifiers.push(m);
        });
    });
...
class TokensProvider implements vscode.DocumentSemanticTokensProvider, vscode.HoverProvider {
    ...
    constructor() {
        ...
        const enabledTerms: string[] = vscode.workspace.getConfiguration("syntax").get("highlightTerms")!;
        availableTerms.forEach(term => {
            if (enabledTerms.includes(term))
                this.supportedTerms.push(term);
        });
        const a = vscode.workspace.getConfiguration("syntax");
